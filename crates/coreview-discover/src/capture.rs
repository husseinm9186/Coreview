...
use crate::backup::{backup_path_named, BackupKind, BackupPathError};
...
pub struct BackupTarget {
    ...
    pub name: String,
    /// Show commands for this device only, run after the global list (LT-149).
    #[serde(default)]
    pub commands: Vec<String>,
    /// The device's site, for a `{site}` in the filename pattern (LT-151).
    #[serde(default)]
    pub site: String,
}

/// Show commands to capture alongside, or instead of, the configurations.
#[derive(Debug, Clone)]
pub struct ShowPlan {
    /// Run on every device, before its own list.
    pub commands: Vec<String>,
    /// What to send first so the output is not paged.
    pub paging: crate::showcmd::Paging,
}
...
pub struct BackupOptions {
    ...
    /// Show commands as well (LT-149). `None` when none were asked for.
    pub show: Option<ShowPlan>,
    /// How captures are named inside each device's folder (LT-151). `None`
    /// is `backup::DEFAULT_PATTERN`.
...
pub fn write_capture(
    ...
    write_capture_named(root, name, address, "", None, stamp, kind, contents)
}

/// `write_capture` with a site and filename pattern (LT-151).
#[allow(clippy::too_many_arguments)]
pub fn write_capture_named(
    root: &Path,
    name: &str,
    address: &str,
    site: &str,
    ...
    let path = backup_path_named(root, name, address, site, stamp, kind, pattern).map_err(describe)?;
...
/// Writes a show-commands capture (LT-149).
///
/// Not held to `looks_like_config`, which would refuse every one of them — a
/// route table is not a configuration and is not meant to be. `unchanged` is
/// always false: show output carries clocks, uptimes and counters, so two
/// captures are never byte-identical and claiming otherwise would be noise.
pub fn write_show_capture(
    root: &Path,
    name: &str,
    address: &str,
    site: &str,
    ...
    stamp: &str,
    contents: &str,
) -> Result<BackupSaved, String> {
    if contents.trim().is_empty() {
        return Err("there was nothing to write".into());
    }
    let path = backup_path_named(root, name, address, site, stamp, BackupKind::ShowCommands, pattern)
        .map_err(describe)?;
    let dir = path.parent().ok_or("the backup path has no folder")?;
    std::fs::create_dir_all(dir).map_err(|e| format!("could not create {}: {e}", dir.display()))?;
    std::fs::write(&path, contents)
        .map_err(|e| format!("could not write {}: {e}", path.display()))?;
    // Show output can carry secrets too — `show running-config` in a list is
    // exactly as sensitive as the backup of it.
    restrict_permissions(&path);
    Ok(BackupSaved {
        name: name.to_string(),
...
pub fn list_captures(root: &Path, name: &str, address: &str) -> Vec<PathBuf> {
    ...
    let mut files: Vec<PathBuf> = entries
        ...
        .collect();
    // Ordered by the stamp inside the name, wherever a filename pattern put it
    // (LT-151) — `{site}_{stamp}` must not sort by site. The name breaks ties
    // and orders anything without a stamp.
    let key = |p: &PathBuf| {
        let f = p.file_name().map(|f| f.to_string_lossy().to_string()).unwrap_or_default();
        (crate::backup::stamp_in(&f).map(str::to_string), f)
    };
...
/// filename pattern decides where `{kind}` goes (LT-151).
pub fn latest_capture(root: &Path, name: &str, address: &str, kind: BackupKind) -> Option<PathBuf> {
    let slug = kind.slug();
    list_captures(root, name, address)
        .into_iter()
        .find(|p| p.file_name().map(|f| f.to_string_lossy().contains(slug)).unwrap_or(false))
}
...
async fn back_up_one(
    ...
    let show = options.show.as_ref().and_then(|plan| {
        let commands = crate::showcmd::plan_for(&plan.commands, &target.commands);
        (!commands.is_empty()).then_some((commands, plan.paging))
    });

    // User mode is only fatal when there is nothing else to do. Most show
    // commands run without enable, so a run that asked for them still has
    // work here even when the configurations do not.
    if !enabled && show.is_none() {
        device.close().await;
        return Err(
    ...
    for kind in &options.kinds {
        ...
        match device.run(command).await {
            ...
            Ok(text) => match write_capture_named(
                &options.root,
                &name,
                &target.address,
                &target.site,
    ...
    if let Some((commands, paging)) = show {
        match capture_show(&mut device, options, &name, target, stamp, &commands, paging, enabled).await {
            Ok(one) => saved.push(one),
            Err(why) => problems.push(format!("show-commands: {why}")),
        }
    }
    device.close().await;
...
/// Runs a device's show commands and files them as one capture (LT-149).
///
/// The read-only check is repeated here as well as at the interface boundary:
/// `run_backups` is a public function, and the promise that nothing in a list
/// changes a device has to hold whoever calls it.
#[allow(clippy::too_many_arguments)]
async fn capture_show(
    device: &mut crate::ssh::Device,
    options: &BackupOptions,
    name: &str,
    target: &BackupTarget,
    stamp: &str,
    commands: &[String],
    paging: crate::showcmd::Paging,
    enabled: bool,
) -> Result<BackupSaved, String> {
    use crate::showcmd::{check_read_only, render, Section};

    for setup in paging.setup_commands() {
        // A platform that does not know its own pager command is not a reason
        // to stop: `--More--` is still answered as it arrives.
        let _ = device.run(setup).await;
    }

    let mut sections = Vec::with_capacity(commands.len());
    for command in commands {
        if let Err(why) = check_read_only(command) {
            sections.push(Section {
                command: command.clone(),
                output: format!("(not sent: {why})"),
                rejected: true,
            });
            continue;
        }
        match device.run(command).await {
            Ok(text) => sections.push(Section {
                rejected: crate::stacking::command_rejected(&text),
                command: command.clone(),
                output: text,
            }),
            Err(e) => sections.push(Section {
                command: command.clone(),
                output: format!("(did not complete: {e})"),
                rejected: true,
            }),
        }
    }

    if sections.iter().all(|s| s.rejected) {
        return Err("the device accepted none of the show commands".into());
    }

    let mut text = render(name, &target.address, stamp, &sections);
    if !enabled {
        text = format!(
            "# Ran in user mode — the device refused enable, so commands that need privilege may be refused\n{text}"
        );
    }
    write_show_capture(
        &options.root,
...
mod tests {
    ...
    /// A pattern that starts with the site must not sort by site, and the
    /// unchanged check must still find the previous capture of the same kind.
    #[test]
    fn captures_named_by_a_pattern_list_newest_first_and_still_spot_an_unchanged_one() {
        let root = temp_root("pattern");
        let pat = Some("{site}_{device}_{stamp}_{kind}");
        // Older capture under a site that sorts last by name.
        write_capture_named(&root, "SW1", "192.0.2.1", "ZZ", pat, "20260828-090000", BackupKind::Running, CONFIG)
            .unwrap();
        write_capture_named(&root, "SW1", "192.0.2.1", "ZZ", pat, "20260829-090000", BackupKind::Startup, CONFIG)
            .unwrap();
        let (newest, unchanged) =
            write_capture_named(&root, "SW1", "192.0.2.1", "AA", pat, "20260830-090000", BackupKind::Running, CONFIG)
                .unwrap();
        assert!(unchanged, "the previous running-config was found under the other name");
        assert_eq!(
            newest.file_name().unwrap().to_string_lossy(),
            "AA_SW1_20260830-090000_running-config.txt"
        );

        let listed: Vec<String> = list_captures(&root, "SW1", "192.0.2.1")
            .iter()
            .map(|p| p.file_name().unwrap().to_string_lossy().to_string())
            .collect();
        assert_eq!(
            ...
            [
                "AA_SW1_20260830-090000_running-config.txt",
                "ZZ_SW1_20260829-090000_startup-config.txt",
                "ZZ_SW1_20260828-090000_running-config.txt",
            ]
        );
        assert!(latest_capture(&root, "SW1", "192.0.2.1", BackupKind::Startup)
            .unwrap()
            .to_string_lossy()
            .contains("20260829"));
    ...
    #[test]
    fn a_bad_pattern_writes_nothing() {
        let root = temp_root("badpattern");
        let err = write_capture_named(&root, "SW1", "", "", Some("{device}"), "20260828-090000", BackupKind::Running, CONFIG)
            .unwrap_err();
        assert!(err.contains("{stamp}"), "{err}");
        assert!(list_captures(&root, "SW1", "").is_empty());
        let _ = std::fs::remove_dir_all(&root);
    }
}
