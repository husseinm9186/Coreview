...
pub fn parse_etherchannel_summary(text: &str) -> Vec<PortChannel> {
    ...
    for line in text.lines() {
        ...
        if fields.first().is_some_and(|f| f.chars().all(|c| c.is_ascii_digit())) {
            ...
            // `skip`, not `[3..]`: a row cut short has no members, and must
            // not panic (LT-283).
            let members = fields.iter().skip(3).filter_map(|p| member(p)).collect();
...
mod tests {
    use super::*;
    /// LT-283: a bundle row cut to two fields — a slow read or a swallowed
    /// paging prompt — panicked slicing its member ports, and took the crawl of
    /// that device with it.
    #[test]
    fn a_bundle_row_cut_short_is_read_not_a_panic() {
        let cut = "Group  Port-channel  Protocol    Ports\n1      Po1(SU)\n";
        let got = parse_etherchannel_summary(cut);
        assert_eq!(got.len(), 1);
        assert!(got[0].members.is_empty());
    }
