...
impl DiscoveryFilter {
    ...
    /// (LT-156) — a device's default-route next hop.
    ///
    /// The same subnet rules as `should_crawl`, with no class to check: a
    /// default gateway is the way out of a network, which makes it
    /// infrastructure by definition. An excluded subnet always wins, and an
    /// address that does not parse is never dialled.
    pub fn allows_address(&self, ip: &str) -> bool {
        if ip.trim().parse::<std::net::Ipv4Addr>().is_err() {
            return false;
        }
        if !self.exclude_subnets.is_empty() && within_any(ip, &self.exclude_subnets) {
            return false;
        }
        self.subnets.is_empty() || within_any(ip, &self.subnets)
    }
...
mod tests {
    ...
    #[test]
    fn a_bare_address_obeys_the_subnet_rules() {
        let open = DiscoveryFilter::default();
        assert!(open.allows_address("192.0.2.1"), "no subnets set means anywhere");
        assert!(!open.allows_address("not-an-address"));
        assert!(!open.allows_address(""));

        let lab = DiscoveryFilter {
            subnets: vec![parse_cidr("192.0.2.0/24").unwrap()],
            exclude_subnets: vec![parse_cidr("192.0.2.128/25").unwrap()],
            ..Default::default()
        };
        assert!(lab.allows_address("192.0.2.1"));
        assert!(!lab.allows_address("198.51.100.1"), "outside the allowed subnets");
        assert!(!lab.allows_address("192.0.2.200"), "an excluded subnet wins");
    }
}
