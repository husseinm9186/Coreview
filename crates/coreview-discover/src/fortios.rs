...
Burn in MAC: e8:1c:ba:00:00:4a
Hostname: S000TESTSERIAL00
Security mode: none
"#;
...
== [ internal ]
name: internal    status: up    mode: static    ip: 192.168.77.203 255.255.255.0   type: physical
== [ 90_lab ]
...
MED type codes:
  ...
  port1       Down     -                           -     -           -         -
  port9       Up       LAB-CORE-SW1.lab.local      120   B,R         -         Gi0/9
  port12      Up       Lab Office AP               120   W           -         eth1
  port13      Down     -                           -     -           -         -
"#;
    ...
    fn reads_the_identity_a_fortiswitch_reports() {
        ...
        assert_eq!(s.hostname.as_deref(), Some("S000TESTSERIAL00"));
        assert_eq!(s.model.as_deref(), Some("FortiSwitch-224E"));
        assert_eq!(s.serial.as_deref(), Some("S000TESTSERIAL00"));
    ...
    fn skips_interfaces_dhcp_never_configured() {
        ...
        assert_eq!(a.len(), 1, "{a:?}");
        assert_eq!(a[0].ip, "192.168.77.203");
    ...
    fn reads_neighbours_and_ignores_empty_ports() {
        ...
        assert_eq!(n[0].short_name, "LAB-CORE-SW1");
        ...
        assert_eq!(n[1].short_name, "Lab Office AP");
        assert_eq!(n[1].class, DeviceClass::AccessPoint);
    }

    #[test]
    fn a_table_with_nothing_attached_yields_nothing() {
        let empty = LLDP.replace("LAB-CORE-SW1.lab.local", "-                     ")
            .replace("Lab Office AP", "-            ");
...
pub fn parse_wtp_status(out: &str) -> Vec<AccessPoint> {
    ...
    for line in out.lines() {
        ...
        if let Some(rest) = t.strip_prefix("WTP:") {
            ...
            in_lldp = false;
            // "LAB_FAP231_0003  0-192.168.77.110:5246" — the name is the
...
fn uplink_from(fields: &[(String, String)]) -> Option<Neighbor> {
    ...
    // "mac 74:ac:b9:00:00:05" — the word says what kind of id it is.
...
";
    ...
    fn reads_every_record() {
        ...
        assert_eq!(found[1].address.as_deref(), Some("192.168.77.71"));
    }
...
mod dhcp_lease_tests {
    ...
    const LEASES: &str = "STAFFWLAN\n  IP\t\tMAC-Address\t\tHostname            VCI                 SSID                AP                  SERVER-ID           Expiry\n  192.168.1.204\tac:3d:cb:00:00:eb\tPC-012              MSFT 5.0            STAFF-IT            PU431F0000000002    22                  Thu Sep  3 07:51:56 2026\n  192.168.1.205\t42:59:ba:00:00:cb\tiPhone                                  STAFF-IT            FP231FTF00000001    22                  Fri Sep  4 21:20:35 2026\nGUESTWLAN\n  IP\t\tMAC-Address\t\tHostname            VCI                 SSID                AP                  SERVER-ID           Expiry\n  172.16.99.21\t1e:6e:5b:00:00:a2\t                                        GUESTS1             PU431F0000000002    21                  Mon Aug 31 10:37:11 2026\n  172.16.99.18\tda:b6:b2:00:00:8d\t                    android-dhcp-14     GUESTS1             PU431F0000000002    21                  Wed Sep  2 07:43:07 2026\n  172.16.99.19\tc4:db:ad:00:00:d3\tDoorbellCamera-0001                     GUESTS1             PU431F0000000002    21                  Sat Sep  5 23:45:43 2026\n";
    ...
    fn reads_a_full_row() {
        ...
        assert_eq!(l.hostname.as_deref(), Some("PC-012"));
        assert_eq!(l.os_name.as_deref(), Some("MSFT 5.0"));
        assert_eq!(l.fortiap_ssid.as_deref(), Some("STAFF-IT"));
        assert_eq!(l.fortiap_name.as_deref(), Some("PU431F0000000002"));
    }
    ...
    fn a_blank_hostname_stays_blank_and_does_not_borrow_the_next_column() {
        ...
        // name. Reading "GUESTS1" as its hostname would name a dozen devices
        // after the SSID they are on.
        let l = &parse_dhcp_leases(LEASES)[2];
        assert_eq!(l.hostname, None);
        assert_eq!(l.fortiap_ssid.as_deref(), Some("GUESTS1"));
    }
    ...
    fn each_lease_carries_the_interface_it_was_served_on() {
        ...
        assert_eq!(all[0].interface.as_deref(), Some("STAFFWLAN"));
        assert_eq!(all[2].interface.as_deref(), Some("GUESTWLAN"));
    }
    ...
    fn the_mac_matches_the_arp_table_spelling() {
        let l = &parse_dhcp_leases(LEASES)[0];
        assert_eq!(l.mac, crate::arp::normalise_mac("AC3D.CB00.00EB").expect("valid"));
    }

    #[test]
    fn no_leases_is_no_devices() {
        assert!(parse_dhcp_leases("").is_empty());
        assert!(parse_dhcp_leases("STAFFWLAN\n").is_empty());
    }
}
...
mod managed_switch_tests {
    ...
    edit "S000TESTSERIAL00"
        set sn "S000TESTSERIAL00"
        set fsw-wan1-peer "Fortilink"
        set fsw-wan1-admin enable
        config ports
            edit "port1"
                set vlan "PRINTERS-01"
            next
            edit "port2"
                set vlan "_default"
            next
...
"#;
    ...
    fn finds_the_switches_it_manages() {
        ...
        assert_eq!(
            ...
            ["S000TESTSERIAL00", "S248EPTF18001234"]
        );
    }
...
mod wtp_status_tests {
    ...
    const WTP: &str = r#"WTP: LAB_FAP231_0003  0-192.168.77.110:5246
    vdom             : root
    vap in user      : no
    wtp-id           : FP231FTF00000001
    region-code      : A 
    name             : LAB_FAP231_0003
    mgmt_vlanid      : 0
    ...
    local-ip-addr   : 192.168.77.110
    board-mac        : 74:78:a6:00:00:68
    ble-mac          : f4:78:a6:00:00:6f
  ...
  LLDP               : enabled (total 1)
    local port       : lan1
    chassis id       : mac 74:ac:b9:00:00:05
    sys name         : LAB-ACC-SW
    ...
    MAU oper type    : 
    ip               : fe80::76ac:b9ff
WTP: LAB-OFFICE-AP-0001  0-192.168.77.114:15246
    vdom             : root
    vap in user      : no
    wtp-id           : PU431F0000000002
    region-code      : A 
    name             : LAB-OFFICE-AP-0001
    mgmt_vlanid      : 0
    data-chan-sec    : clear-text
    mesh-uplink      : ethernet
    mesh-hop-count   : 0
    ...
    local-ip-addr   : 192.168.77.114
    board-mac        : 00:0c:e6:00:00:a0
    ble-mac          : 00:00:00:00:00:00
    join-time        : Tue Aug 18 22:05:42 2026
    connection-state : Connected
    image-download-progress: 0
WTP: LAB_AP431F_0002  0-192.168.77.141:5246
    vdom             : root
    vap in user      : no
    wtp-id           : PU431F0000000003
    region-code      : A 
    name             : LAB_AP431F_0002
    mgmt_vlanid      : 0
    data-chan-sec    : clear-text
    mesh-uplink      : ethernet
    mesh-hop-count   : 0
    parent-wtp-id    : 
    software-version : PU431F-v7.0.6-build0159
    local-ip-addr   : 192.168.77.141
    board-mac        : 00:0c:e6:00:01:a0
    ble-mac          : 00:00:00:00:00:00
...
"#;
    ...
    fn finds_every_access_point() {
        ...
        assert_eq!(
            ...
            ["LAB_FAP231_0003", "LAB-OFFICE-AP-0001", "LAB_AP431F_0002"]
        );
    }
    ...
    fn reads_the_address_the_ap_is_actually_on() {
        ...
        let aps = parse_wtp_status(WTP);
        assert_eq!(aps[0].address.as_deref(), Some("192.168.77.110"));
        assert_eq!(aps[2].address.as_deref(), Some("192.168.77.141"));
    }

    #[test]
    fn reads_the_serial_and_state() {
        let aps = parse_wtp_status(WTP);
        assert_eq!(aps[0].serial.as_deref(), Some("FP231FTF00000001"));
    ...
    fn finds_what_the_ap_is_plugged_into() {
        ...
        assert_eq!(up.short_name, "LAB-ACC-SW");
    ...
    fn the_uplink_mac_is_kept_so_arp_can_resolve_it() {
        let up = parse_wtp_status(WTP)[0].uplink.clone().expect("an uplink");
        assert_eq!(up.chassis_id, crate::arp::normalise_mac("74:ac:b9:00:00:05"));
    ...
    fn an_ap_with_no_lldp_reports_no_uplink() {
        ...
        assert_eq!(aps[1].address.as_deref(), Some("192.168.77.114"));
    }
