...
/// `entPhysicalSerialNum` — ENTITY-MIB, the *standard* place a device keeps
/// the serial of every physical thing in it (LT-134).
///
/// Worth knowing before anyone goes shopping for vendor MIBs: this is not a
/// Cisco object or an Aruba object, it is RFC 4133, and Cisco, Aruba, Juniper,
/// Dell and HP all answer it. It gives a serial **per chassis**, so a stack
/// reports one per member — which is exactly what an RMA is raised against
/// and what the operator asked for twice.
const ENT_PHYSICAL_SERIAL: &[u64] = &[1, 3, 6, 1, 2, 1, 47, 1, 1, 1, 1, 11];

/// `entPhysicalModelName` — the same table's model column.
const ENT_PHYSICAL_MODEL: &[u64] = &[1, 3, 6, 1, 2, 1, 47, 1, 1, 1, 1, 13];

/// `entPhysicalClass` — what each row of that table *is*.
///
/// Needed because the table lists everything physical: measured against the
/// operator's own 2960CX it returns the chassis serial **and** the power
/// supply's serial. A power supply is not what an RMA for the switch is
/// raised against, so only chassis rows are kept.
const ENT_PHYSICAL_CLASS: &[u64] = &[1, 3, 6, 1, 2, 1, 47, 1, 1, 1, 1, 5];

/// `entPhysicalClass` value for a chassis. RFC 4133's enumeration; a stack
/// reports one row per member, which is the thing worth having.
const ENT_CLASS_CHASSIS: i64 = 3;

/// How many rows of a table to read before giving up.
///
/// A chassis switch enumerates every port, fan and transceiver in
/// ENTITY-MIB, so this table is hundreds of rows on real hardware and almost
/// all of them have no serial. The cap is generous enough for a full chassis
/// and small enough that a device answering nonsense cannot hold a crawl.
...
pub struct SnmpIdentity {
    ...
    pub bridges: bool,
    /// Every chassis serial the device reports through ENTITY-MIB (LT-134).
    /// One per member on a stack; empty where the device does not implement
    /// the MIB, which is not the same as having no serial.
    #[serde(default)]
    pub serials: Vec<String>,
    /// The model names from the same table, in the same order.
    #[serde(default)]
...
pub async fn identify(
    ...
    // Boxed, so the session-setup future is not carried inline. The deeper
    ...
    // LT-134: the serials, from the standard ENTITY-MIB rather than from any
    // vendor's own. Best effort: a device that does not implement the MIB
    // simply reports none, which is not the same as having no serial and is
    // not a reason to fail an identification that otherwise worked.
    let chassis = walk_chassis_rows(&mut session, timeout).await;
    identity.serials =
        walk_column(&mut session, ENT_PHYSICAL_SERIAL, &chassis, timeout).await;
    identity.models = walk_column(&mut session, ENT_PHYSICAL_MODEL, &chassis, timeout).await;
...
/// A session ready for requests: v2c as it is, v3 with engine discovery done
/// and the other key-extension method tried before giving up. Shared by
/// `identify` and `arp_table` (LT-124) so both authenticate the same way.
...
/// Reads one column of a table, as the strings in it.
///
/// GETNEXT rather than GETBULK: bulk is faster and is a v2c-and-later
/// optimisation that some of the older kit this is pointed at answers badly,
/// and a column of a few hundred rows is not worth the risk of a device that
/// mishandles it.
///
/// **Stops at the end of the column.** A walk that does not check the prefix
/// runs off the end of the table and keeps going through the rest of the MIB,
/// which on a chassis switch is tens of thousands of objects.
///
/// Empty on any error, deliberately: this is extra information on top of an
/// identification that has already succeeded, and losing it must never turn a
/// reachable device into an unreachable one.
async fn walk_column(
    ...
    let mut out = Vec::new();
    for (row, value) in walk(session, column, timeout).await {
        if !only_rows.is_empty() && !only_rows.contains(&row) {
            continue;
        }
        if let Walked::Octets(bytes) = value {
            let text = String::from_utf8_lossy(&bytes).trim().to_string();
            // Most rows of ENTITY-MIB are fans and transceivers carrying no
            // serial at all, and an empty string is not one.
...
/// The row indices of everything the device calls a chassis.
async fn walk_chassis_rows(
    session: &mut snmp2::AsyncSession,
    timeout: std::time::Duration,
) -> Vec<String> {
    walk(session, ENT_PHYSICAL_CLASS, timeout)
        .await
        .into_iter()
        .filter(|(_, v)| matches!(v, Walked::Int(n) if *n == ENT_CLASS_CHASSIS))
        .map(|(row, _)| row)
        .collect()
}

/// Walks one column and yields `(row index, value)`.
///
/// GETNEXT rather than GETBULK: bulk is a later optimisation that some of the
/// older kit this is pointed at answers badly, and a column of a few hundred
/// rows is not worth the risk of a device that mishandles it.
///
/// **Stops at the end of the column.** A walk that does not check the prefix
/// runs off the end of the table and carries on through the rest of the MIB,
/// which on a chassis switch is tens of thousands of objects.
///
/// Empty on any error, deliberately: this is extra on top of an
/// identification that already succeeded, and losing it must never turn a
/// reachable device into an unreachable one.
async fn walk(
    session: &mut snmp2::AsyncSession,
    column: &[u64],
    ...
    walk_limited(session, column, timeout, MAX_TABLE_ROWS).await
}

/// `walk` with its own row limit, for a table that is legitimately larger
/// than ENTITY-MIB — a router's ARP table (LT-124).
pub(crate) async fn walk_limited(
    session: &mut snmp2::AsyncSession,
    column: &[u64],
    timeout: std::time::Duration,
    max_rows: usize,
) -> Vec<(String, Walked)> {
    let Ok(root) = Oid::from(column) else { return Vec::new() };
    let prefix = format!("{}.", root);
    let mut out = Vec::new();
    let mut at = root;
    let mut previous: Vec<u64> = column.to_vec();

    for _ in 0..max_rows {
        let Ok(Ok(mut pdu)) = tokio::time::timeout(timeout, session.getnext(&at)).await else {
            break;
        };
        let Some((oid, value)) = pdu.varbinds.next() else { break };
        let name = oid.to_string();
        if !name.starts_with(&prefix) {
            break;
        }
        let parts: Vec<u64> = name.split('.').filter_map(|p| p.parse::<u64>().ok()).collect();
        // A broken agent can answer GETNEXT with an OID that is not after the
        // one asked for, and following it loops until the row limit. Stop.
        if !oid_advances(&previous, &parts) {
            break;
        }
        let row = name[prefix.len()..].to_string();
        out.push((row, owned(value)));
        let Ok(next) = Oid::from(&parts[..]) else { break };
        previous = parts;
        at = next;
    }
    out
...
/// ipNetToMediaPhysAddress (RFC 1213): a device's ARP table, one row per
/// neighbour, indexed `ifIndex.a.b.c.d`. Measured on the lab FortiGate before
/// this was written: 36 rows in exactly that shape, each a 6-byte MAC, while
/// its RFC 4293 table (ipNetToPhysical) was empty — so the older table is the
/// one to read.
const IP_NET_TO_MEDIA_PHYS_ADDRESS: &[u64] = &[1, 3, 6, 1, 2, 1, 4, 22, 1, 2];

/// A core router's ARP table can hold thousands of neighbours; ENTITY-MIB's
/// limit would silently cut it short.
const MAX_ARP_ROWS: usize = 16_384;

/// One neighbour a device has resolved.
#[derive(Debug, Clone, PartialEq, Eq, serde::Serialize)]
pub struct ArpEntry {
    pub ip: std::net::Ipv4Addr,
    /// Lower-case, colon-separated, like the sweep's own.
    pub mac: String,
}

/// A device's ARP table, over SNMP (LT-124). Used on a gateway to give MAC
/// addresses to hosts on subnets a sweep can only reach through a router.
///
/// The first request is made directly rather than through `walk`, which
/// swallows errors: a wrong v3 user or algorithm has to come back as that,
/// not as an empty table that reads like a router with no neighbours.
pub async fn arp_table(
    ...
    // caller's future small as well, for one allocation per device.
    let mut session = Box::pin(open_session(host, port, auth, timeout)).await?;
    let root = Oid::from(IP_NET_TO_MEDIA_PHYS_ADDRESS).map_err(|_| SnmpError::NothingReturned { host: host.into() })?;
    tokio::time::timeout(timeout, session.getnext(&root))
        .await
        .map_err(|_| SnmpError::Timeout { host: host.into(), timeout })?
        .map_err(|e| describe_v3_failure(host, port, auth, e))?;

    let mut out: Vec<ArpEntry> = Vec::new();
    for (row, value) in walk_limited(&mut session, IP_NET_TO_MEDIA_PHYS_ADDRESS, timeout, MAX_ARP_ROWS).await {
        if let Walked::Octets(bytes) = value {
            if let Some(entry) = arp_entry(&row, &bytes) {
                // One address on two interfaces is one neighbour.
...
/// One ARP row: `ifIndex.a.b.c.d` and six bytes. `None` for an incomplete
/// entry (no MAC yet), an all-zero or broadcast MAC, or a row of another shape.
fn arp_entry(row: &str, bytes: &[u8]) -> Option<ArpEntry> {
    let parts: Vec<&str> = row.split('.').collect();
    // An interface index and four address octets, all numbers — a row whose
    // first part is not an index is not this table's shape.
    if parts.len() != 5 || parts[0].parse::<u32>().is_err() {
        return None;
    }
    let octets: Vec<u8> = parts[1..].iter().map(|p| p.parse::<u8>().ok()).collect::<Option<_>>()?;
...
/// Whether `next` comes after `previous` in OID order — what GETNEXT promises
/// and a broken agent does not always deliver.
fn oid_advances(previous: &[u64], next: &[u64]) -> bool {
    next > previous
}

/// One column of any table, read with a credential (LT-134): rows as
/// `(index after the column, value)`, in the device's order. The first request
/// is made directly, so a wrong credential is reported rather than read as an
/// empty table. Opens its own session; a caller reading several columns of one
/// device should prefer a shared one.
pub async fn walk_table(
    ...
    column: &[u64],
    max_rows: usize,
) -> Result<Vec<(String, Walked)>, SnmpError> {
    let mut session = Box::pin(open_session(host, port, auth, timeout)).await?;
    let root = Oid::from(column).map_err(|_| SnmpError::NothingReturned { host: host.into() })?;
    tokio::time::timeout(timeout, session.getnext(&root))
        .await
        .map_err(|_| SnmpError::Timeout { host: host.into(), timeout })?
        .map_err(|e| describe_v3_failure(host, port, auth, e))?;
    Ok(walk_limited(&mut session, column, timeout, max_rows).await)
}

/// A walked value, owned.
///
/// The PDU's `Value` borrows from the response buffer, so it cannot outlive
/// one iteration. Copying the two kinds this reads into a small owned enum is
/// the honest way to carry them out — the first version reached for
/// `Box::leak`, which compiles and leaks a little memory for every row of
/// every table on every device a crawl touches.
...
mod tests {
    ...
    fn nothing_listening_says_so_instead_of_socket_receive_error() {
        ...
        for source in [snmp2::Error::Receive, snmp2::Error::Send] {
            let err = describe_v3_failure("192.168.77.195", 161, &auth, source);
            assert!(matches!(err, SnmpError::NoListener { .. }), "{err}");
            let msg = err.to_string();
            assert!(msg.contains("192.168.77.195:161"), "{msg}");
...
/// LT-124: reading an ARP table's rows, in the shape the lab FortiGate
/// returned them. Addresses and MACs invented (RFC 5737, a made-up MAC).
#[cfg(test)]
mod arp_tests {
    use super::*;

    const MAC: [u8; 6] = [0x00, 0x0C, 0x29, 0xAA, 0xBB, 0xCC];

    #[test]
    fn a_row_gives_the_address_and_the_mac() {
        let e = arp_entry("11.192.0.2.10", &MAC).expect("an entry");
        assert_eq!(e.ip, std::net::Ipv4Addr::new(192, 0, 2, 10));
        assert_eq!(e.mac, "00:0c:29:aa:bb:cc");
    }

    #[test]
    fn an_incomplete_zero_or_broadcast_entry_is_not_a_neighbour() {
        assert_eq!(arp_entry("11.192.0.2.10", &[]), None, "no MAC resolved yet");
        assert_eq!(arp_entry("11.192.0.2.10", &[0; 6]), None);
        assert_eq!(arp_entry("11.192.0.2.10", &[0xFF; 6]), None);
        assert_eq!(arp_entry("11.192.0.2.10", &MAC[..5]), None, "not six bytes");
    }

    #[test]
    fn a_row_of_another_shape_is_refused() {
        assert_eq!(arp_entry("192.0.2.10", &MAC), None, "no interface index");
        assert_eq!(arp_entry("11.1.192.0.2.10", &MAC), None, "the RFC 4293 index shape");
        assert_eq!(arp_entry("11.300.0.2.10", &MAC), None, "an octet out of range");
        assert_eq!(arp_entry("eleven.192.0.2.10", &MAC), None);
    }
}

/// LT-134: a walk stops at an agent that does not advance.
#[cfg(test)]
mod walk_tests {
    use super::*;

    #[test]
    fn an_oid_after_the_last_advances() {
        assert!(oid_advances(&[1, 3, 6, 1], &[1, 3, 6, 1, 1]));
        assert!(oid_advances(&[1, 3, 6, 1, 2], &[1, 3, 6, 1, 10]));
    }

    #[test]
    fn the_same_or_an_earlier_oid_does_not() {
        assert!(!oid_advances(&[1, 3, 6, 1, 2], &[1, 3, 6, 1, 2]));
        assert!(!oid_advances(&[1, 3, 6, 1, 2], &[1, 3, 6, 1, 1, 9]));
    }
}
