...
/// LT-262: the key's bytes, to be kept in the operating system's keychain when
/// the operator asks for that — the key rather than the passphrase, so a
/// passphrase used anywhere else is never written down. Zeroed when dropped.
pub fn key_bytes(key: &VaultKey) -> zeroize::Zeroizing<Vec<u8>> {
    zeroize::Zeroizing::new(key.0.to_vec())
}

/// LT-262: a key read back from the keychain, accepted only if it opens this
/// vault's verifier — a key kept for a vault since discarded opens nothing.
pub fn unlock_with_key(bytes: &[u8], header: &VaultHeader) -> Result<VaultKey, VaultError> {
    let raw: [u8; KEY_LEN] = bytes.try_into().map_err(|_| VaultError::WrongPassphrase)?;
    verify(VaultKey(raw), header)
}

fn verify(key: VaultKey, header: &VaultHeader) -> Result<VaultKey, VaultError> {
...
mod tests {
    ...
    fn a_secret_moves_between_vaults_by_resealing() {
        ...
        let from_them = seal(&exporting, "not-a-real-secret-fixture").unwrap();
        ...
        assert_eq!(open(&importing, &ours).unwrap(), "not-a-real-secret-fixture");
    ...
    fn a_secret_survives_a_round_trip() {
        ...
        let sealed = seal(&key, "not-a-real-secret-fixture").unwrap();
        assert_eq!(open(&key, &sealed).unwrap(), "not-a-real-secret-fixture");
    }
    ...
    fn secrets_with_awkward_characters_survive() {
        ...
        let (_, key) = create(PASSPHRASE).unwrap();
        for secret in ["not-a-real-secret-fixture", "p@$$ w/ spaces", "unicode ✓ é", "", "\n\t"] {
            let sealed = seal(&key, secret).unwrap();
