...
pub async fn run_once(cfg: &ProbeConfig) -> ProbeResult {
    ...
    match cfg.kind {
        ...
        // LT-219: a named server, asked directly.
        ProbeKind::Dns if cfg.dns_server.as_deref().is_some_and(|s| !s.trim().is_empty()) => {
            crate::dnsquery::probe_dns_query(
                &cfg.id,
                &cfg.target,
                cfg.dns_server.as_deref().unwrap_or_default(),
                cfg.dns_record.as_deref().unwrap_or("A"),
        ...
        ProbeKind::Udp => {
            crate::udp::probe_udp(&cfg.id, &cfg.target, cfg.tcp_port.unwrap_or(0), cfg.udp_payload.as_deref(), cfg.timeout_ms, t)
                .await
        }
        ProbeKind::Snmp => crate::snmpcheck::run(cfg, t).await,
