...
/// simple.
pub(crate) async fn read_capped<S: AsyncRead + Unpin>(stream: &mut S, cap: usize) -> std::io::Result<Vec<u8>> {
    let mut buf = Vec::new();
...
/// Shared with `banner` (LT-124), which asks the same one question.
pub(crate) fn request_line(host: &str, path: &str) -> String {
...
/// Shared with `certfetch` (LT-124), which reads a certificate rather than
/// trusting it and so always uses the accept-any verifier.
pub(crate) fn tls_config(ignore_cert_errors: bool) -> Arc<rustls::ClientConfig> {
