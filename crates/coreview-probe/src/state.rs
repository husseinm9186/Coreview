...
impl ProbeState {
    ...
    pub fn apply(&mut self, cfg: &ProbeConfig, result: &ProbeResult) -> Option<StatusTransition> {
        ...
        if result.outcome.is_success() {
            ...
            // LT-220: it answered, but it restarted since the last check.
            let candidate = if over_warning || result.outcome == crate::types::Outcome::Restarted {
...
mod tests {
    ...
    /// LT-220: a device that restarted answered, so it is not down — but it
    /// is not simply healthy either until the next check sees it running.
    #[test]
    fn a_restart_is_a_warning_then_clears() {
        let mut s = ProbeState::default();
        s.apply(&cfg(), &ok(1, 2.0));
        let mut restarted = ok(2, 2.0);
        restarted.outcome = crate::types::Outcome::Restarted;
        let t = s.apply(&cfg(), &restarted).expect("transition");
        assert_eq!(t.current, HealthStatus::Warning);
        let t = s.apply(&cfg(), &ok(3, 2.0)).expect("transition");
        assert_eq!(t.current, HealthStatus::Healthy);
    }
