...
real network (192.168.77.0/24) wherever a real device could answer. Nine
...
| Ping sweep, two subnets | **PASS**. 192.168.77.0/24 + 10.2.80.0/30 as one progress sequence, "20 of 256 answered", real RTTs, no ping left behind |
...
Five more harnesses join it, each driving the real application above a stubbed
Tauri bridge: `discover.mjs` (the ping sweep, fed the rows a live run against
192.168.77.0/24 actually produced), `join.mjs` (a sweep and a crawl building
one diagram rather than two), `scansettings.mjs` (a scan is remembered and no
secret is — it types five distinctive secrets and asserts none reaches the
settings table), `change.mjs` and `library.mjs`.
