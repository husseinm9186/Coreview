...
// checked rather than assumed still to work.
{
  ...
  if (handle) {
    ...
    // LT-130: a link drawn by hand takes the document's default path, which
    // is Bezier unless the operator saved something else. It used to name
    // smooth-step itself and so could never follow the default at all.
    const drawn = await page.evaluate(() => {
      const d = window.__cvStore.getState().doc;
      const pages = d.pages ? Object.values(d.pages) : [{ edges: d.edges }];
      const edges = pages.flatMap((pg) => pg.edges ?? []);
      return edges.length ? edges[edges.length - 1].data?.pathType : null;
    });
    check("a link drawn by hand defaults to Bezier", drawn === "bezier", String(drawn));
  }
...
// clearing the user's own folder.
{
  ...
  await page.evaluate(() => {
    window.__cvStore.setState({ bundledIcons: [
      { id: "edge-router", name: "Edge Router", category: "Routing WAN",
        svg: '<svg viewBox="0 0 10 10"><rect x="1" y="1" width="8" height="8"/></svg>' },
      { id: "leaf-switch", name: "Leaf Switch", category: "Data Center",
  ...
  await page.locator(".cv-palette-search").fill("leaf");
  await page.waitForTimeout(300);
  check("search reaches the built-in shapes",
    (await section.locator(".cv-palette-item", { hasText: "Leaf Switch" }).count()) === 1 &&
      (await section.locator(".cv-palette-item", { hasText: "Edge Router" }).count()) === 0);
