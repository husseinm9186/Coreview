#!/usr/bin/env node
/**
 * PPTX stencil deck -> <out-dir>/<category>/<slug>.svg
 *
 * For a deck you have the right to use, converted into a folder of your own
 * that you point the icon library at. The output is never committed or
 * bundled: vendor artwork stays out of the repository and the installer
 * (D-028), and `.gitignore` refuses it under `stencils/`.
 *
