...
pub struct IconEntry {
    ...
    pub svg: String,
    /// What the folder's `coreview-stencils.json` says this shape is, and
    /// under what licence (LT-169). Absent when there is no manifest or it
    /// does not list this file.
    #[serde(skip_serializing_if = "Option::is_none")]
    pub meta: Option<crate::stencil_manifest::StencilMeta>,
}
...
fn is_paperwork(path: &Path, ext: &str) -> bool {
    ...
    // converted set — a way to eyeball what was converted, not a shape that
    // failed to load. (Vendor sets are no longer shipped: D-028.)
...
pub fn scan_excluding(dir: &str, skip: &[String]) -> Result<IconLibrary, String> {
    ...
    // LT-169: a manifest that is refused is reported and ignored — the shapes
    // still load, only undescribed.
    let manifest = match crate::stencil_manifest::load(&root) {
    ...
    // The manifest's name and category win over index.json's; its description
    // travels with the icon.
    let describe = |file_name: &str, name: String, category: String| {
        let entry = manifest.get(file_name);
        (
            entry.and_then(|e| e.name.clone()).unwrap_or(name),
            entry.and_then(|e| e.category.clone()).unwrap_or(category),
            entry.map(|e| e.meta.clone()),
        )
    };
    ...
    for path in entries {
        ...
        let (name, category, meta) = describe(&file_name, name, category);
    ...
    if !found.visio.is_empty() {
        if !crate::shapeconv::libvisio_available() {
            ...
            'files: for src in &found.visio {
                ...
                let many = svgs.len() > 1;
                let base = stem(src);
                let (given_name, category, meta) =
                    describe(&file_name, String::new(), folder_category(&roots, src));
                // A manifest names the stencil; each master is numbered under it.
                let base_name = if given_name.is_empty() { display_name("", &base) } else { given_name };
                for (n, svg) in svgs.iter().enumerate() {
                    ...
                    let name = if many { format!("{base_name} {}", n + 1) } else { base_name.clone() };
                    icons.push(IconEntry {
                        id,
                        name,
...
mod folder_tests {
    ...
    /// D-028 (superseding D-022): the repo's `stencils/` folder is still
    /// bundled as a resource, and it must carry no artwork — no vendor's icons,
    /// logos or stencil packs, in the repository or the installer. This scans
    /// that exact folder, so artwork reappearing fails CI rather than shipping.
    /// LT-169: a manifest in the folder describes the shapes it lists, and a
    /// refused one is reported by name while the shapes still load.
    #[test]
    fn a_manifest_describes_its_shapes_and_a_bad_one_is_reported() {
        let dir = tempfile::tempdir().expect("tempdir");
        let svg = r#"<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 4 4"><rect width="4" height="4"/></svg>"#;
        std::fs::write(dir.path().join("edge.svg"), svg).unwrap();
        std::fs::write(dir.path().join("other.svg"), svg).unwrap();
        std::fs::write(
            dir.path().join(crate::stencil_manifest::MANIFEST_FILE),
            r#"{"coreviewStencils":1,"licence":"Drawn for the test","shapes":[
                {"file":"edge.svg","name":"Edge router","category":"Routing","class":"router","vendor":"Example Networks","ports":8,"portNaming":"Port {n}","rackUnits":1}
            ]}"#,
        )
        .unwrap();
        let lib = super::scan(dir.path().to_str().unwrap()).expect("scan");
        assert!(lib.skipped.is_empty(), "{:?}", lib.skipped);
        assert_eq!(lib.icons.len(), 2, "the manifest is paperwork, not a shape");
        let edge = lib.icons.iter().find(|i| i.id == "edge").expect("edge");
        assert_eq!(edge.name, "Edge router");
        assert_eq!(edge.category, "Routing");
        let meta = edge.meta.as_ref().expect("described");
        assert_eq!(meta.class.as_deref(), Some("router"));
        assert_eq!(meta.licence, "Drawn for the test");
        let json = serde_json::to_value(edge).unwrap();
        assert_eq!(json["meta"]["portNaming"], "Port {n}");
        let other = lib.icons.iter().find(|i| i.id == "other").expect("other");
        assert!(other.meta.is_none());
        assert!(serde_json::to_value(other).unwrap().get("meta").is_none());

        std::fs::write(
            dir.path().join(crate::stencil_manifest::MANIFEST_FILE),
            r#"{"coreviewStencils":1,"licence":"x","shapes":[{"file":"edge.svg","class":"toaster"}]}"#,
        )
        .unwrap();
        let lib = super::scan(dir.path().to_str().unwrap()).expect("scan");
        assert_eq!(lib.icons.len(), 2, "a refused manifest still leaves the shapes");
        assert!(lib.icons.iter().all(|i| i.meta.is_none()));
        assert_eq!(lib.skipped.len(), 1, "{:?}", lib.skipped);
        assert!(lib.skipped[0].starts_with("coreview-stencils.json: shape 1 (edge.svg)"), "{:?}", lib.skipped);
        assert!(lib.skipped[0].contains("toaster"));
    }

    #[test]
    fn the_shipped_stencils_folder_carries_no_artwork() {
        let dir = std::path::Path::new(env!("CARGO_MANIFEST_DIR")).join("../stencils");
        let lib = super::scan(dir.to_str().expect("path")).expect("scan");
        assert!(lib.icons.is_empty(), "shipped stencils carry artwork: {:?}",
            lib.icons.iter().map(|i| &i.id).collect::<Vec<_>>());
        assert!(lib.skipped.is_empty(), "shipped stencils carry unreadable files: {:?}", lib.skipped);
        let mut files: Vec<String> = std::fs::read_dir(&dir)
            .expect("the stencils folder exists; the installer bundles it")
            .map(|e| e.expect("entry").file_name().to_string_lossy().into_owned())
            .collect();
        files.sort();
        assert_eq!(files, ["README.md"], "only the README belongs in stencils/");
    }
    ...
    ///
    /// The stencil is a vendor's artwork, so it is not committed (D-028,
    /// LT-162): it is read from `COREVIEW_VSS_FIXTURE` on the operator's
    /// machine, and the assertions below are about that exact file. It never
    /// ran in CI — CI has no libvisio — so moving it out costs no coverage.
    #[test]
    fn a_real_operator_stencil_becomes_drawable_icons() {
        ...
        let Some(fixture) = std::env::var_os("COREVIEW_VSS_FIXTURE")
            .map(std::path::PathBuf::from)
            .filter(|p| p.is_file())
        else {
            eprintln!(
                "skipping: set COREVIEW_VSS_FIXTURE to the operator's local tripp-lite-racks.vss \
                 (vendor stencils are not committed, D-028)"
            );
            return;
        };
        let dir = tempfile::tempdir().expect("tempdir");
