...
pub struct VaultStatus {
    ...
    /// LT-262: whether this machine keeps the vault key in its keychain.
    pub kept_in_keychain: bool,
}

/// LT-262: the setting that says the key is kept in the keychain, so the
...
pub fn discard_vault(state: State<'_, AppState>) -> CmdResult<usize> {
    ...
    *state.vault_key.lock().map_err(db_err)? = None;
    // LT-262: a key kept for the vault that is gone goes with it.
    if kept_in_keychain(&conn)? {
