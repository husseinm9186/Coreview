...
pub struct ImportedDevice {
    ...
    pub width: f64,
    pub height: f64,
    /// LT-243: how its name was set in the drawing, where it said.
    pub label_style: Option<TextStyle>,
}

/// LT-243: a label's formatting, from the shape's first `Character` and
/// `Para` rows.
...
pub struct TextStyle {
    ...
    pub size: Option<f64>,
    pub color: String,
    /// `left`, `center` or `right`; empty where the drawing left it.
    pub align: String,
}
...
pub struct ImportedLink {
    ...
    pub color: String,
    /// LT-243: the bends the connector was routed through, in inches from the
    /// page's bottom-left like a device's pin — the points between its two
    /// ends. Empty for a straight run.
...
struct RawShape {
    ...
    line_colour: String,
    /// LT-243: a 1-D shape's drawn path on the page, begin to end.
    path: Vec<(f64, f64)>,
...
/// The bends between a connector's two ends. Only a path that starts and
/// finishes on the connector's own ends is trusted: one whose rows were partly
/// inherited from a master this does not read would put bends in the wrong
/// place, and a straight line is the honest fallback.
fn bends(c: &RawShape) -> Vec<[f64; 2]> {
    let (Some(begin), Some(end), Some(first), Some(last)) = (c.begin, c.end, c.path.first(), c.path.last()) else { return Vec::new() };
    let close = |a: (f64, f64), b: (f64, f64)| (a.0 - b.0).hypot(a.1 - b.1) < 1e-3;
    if c.path.len() <= 2 || !close(begin, *first) || !close(end, *last) {
        return Vec::new();
    }
    c.path[1..c.path.len() - 1].iter().map(|(x, y)| [*x, *y]).collect()
}

/// The first row of a named section, where a shape has one.
fn section_row<'a, 'i>(node: roxmltree::Node<'a, 'i>, section: &str) -> Option<roxmltree::Node<'a, 'i>> {
    node.children()
        .find(|c| c.has_tag_name("Section") && c.attribute("N") == Some(section))?
        .children()
        .find(|r| r.has_tag_name("Row") && r.attribute("Del") != Some("1"))
}

/// LT-243: a shape's text formatting. Only what the shape itself states —
/// colour, size, bold and italic from `Character`, alignment from `Para` —
/// because what it inherits from a style sheet is the drawing's default, not a
/// choice worth carrying over. A colour written as `THEMEGUARD(RGB(255,0,0))`
/// still has its value in `V`, which is what is read.
fn text_style(shape: roxmltree::Node) -> Option<TextStyle> {
    let mut style = TextStyle::default();
    if let Some(row) = section_row(shape, "Character") {
        let bits = cell(row, "Style").and_then(|v| v.parse::<f64>().ok()).unwrap_or(0.0) as u32;
        style.bold = bits & 1 != 0;
        style.italic = bits & 2 != 0;
        style.size = cell(row, "Size").and_then(|v| v.parse::<f64>().ok()).filter(|v| *v > 0.0).map(|inches| (inches * 72.0 * 10.0).round() / 10.0);
        style.color = cell(row, "Color").map(|v| line_colour(&v)).unwrap_or_default();
    }
    if let Some(row) = section_row(shape, "Para") {
        style.align = match cell(row, "HorzAlign").as_deref() {
            Some("0") => "left",
            Some("1") => "center",
            Some("2") => "right",
            _ => "",
...
/// LT-243: a 1-D shape's path in page inches. Its `Geometry` rows are in the
/// shape's own coordinates, measured from the corner its `LocPin` offsets;
/// turning them onto the page is the pin, minus the local pin, rotated by
/// `Angle`. Checked against connectors saved by Visio, where the first row
/// lands exactly on `BeginX`/`BeginY`. A row that omits a coordinate inherits
/// it from the master — zero for the start of a connector's run, the previous
/// point's value along it — and a row marked `Del` is not drawn. A page shape
/// can override only some rows and inherit the rest, the `MoveTo` included:
/// one Visio-saved connector states nothing but its `LineTo`. The start is
/// then the local origin, which is where a connector master begins.
fn shape_path(shape: roxmltree::Node) -> Vec<(f64, f64)> {
    let num = |n: roxmltree::Node, name: &str| cell(n, name).and_then(|v| v.parse::<f64>().ok());
    let (Some(pin_x), Some(pin_y)) = (num(shape, "PinX"), num(shape, "PinY")) else { return Vec::new() };
    let loc_x = num(shape, "LocPinX").unwrap_or(0.0);
    let loc_y = num(shape, "LocPinY").unwrap_or(0.0);
    let (sin, cos) = num(shape, "Angle").unwrap_or(0.0).sin_cos();
    let Some(geometry) = shape.children().find(|c| c.has_tag_name("Section") && c.attribute("N") == Some("Geometry")) else {
        return Vec::new();
    };
    let mut out = Vec::new();
    let mut last = (0.0, 0.0);
    let to_page = |(lx, ly): (f64, f64)| {
        let (dx, dy) = (lx - loc_x, ly - loc_y);
        (pin_x + dx * cos - dy * sin, pin_y + dx * sin + dy * cos)
    };
    let starts_with_move = geometry.children().find(|r| r.has_tag_name("Row") && r.attribute("Del") != Some("1")).is_some_and(|r| r.attribute("T") == Some("MoveTo"));
    if !starts_with_move {
        out.push(to_page((0.0, 0.0)));
    }
    for row in geometry.children().filter(|r| r.has_tag_name("Row") && r.attribute("Del") != Some("1")) {
        let kind = row.attribute("T").unwrap_or("");
        if !matches!(kind, "MoveTo" | "LineTo" | "ArcTo" | "EllipticalArcTo" | "PolylineTo" | "NURBSTo" | "SplineStart" | "SplineKnot") {
            continue;
        }
        let first = out.is_empty();
        let lx = num(row, "X").unwrap_or(if first { 0.0 } else { last.0 });
        let ly = num(row, "Y").unwrap_or(if first { 0.0 } else { last.1 });
        last = (lx, ly);
        out.push(to_page((lx, ly)));
    }
    out
}
    ...
    for shape in node.children().filter(|c| c.has_tag_name("Shape")) {
        ...
        if !id.is_empty() {
            ...
            out.push(RawShape {
                ...
                path: if bx.is_some() && ex.is_some() { shape_path(shape) } else { Vec::new() },
...
pub fn parse_page(xml: &str, page_name: &str) -> Result<(ImportedPage, Vec<String>), String> {
    ...
    for id in wanted {
        ...
        let (caption, style) = if could_be_a_name(&s.text) {
            (s.text.clone(), s.text_style.clone())
        } else {
            caption_for
                .get(id.as_str())
                .and_then(|c| by_id.get(*c))
                .map(|c| (c.text.clone(), c.text_style.clone()))
                .unwrap_or_default()
        };
...
mod tests {
    ...
    /// LT-243: a page with two boxes and a connector routed round a corner,
    /// written the way Visio writes one: the run in local coordinates, a row
    /// that omits a coordinate, a deleted row, and glue. The straight part of
    /// the transform is the one measured on a Visio-saved connector — its
    /// first row lands on `BeginX`/`BeginY`.
    const ROUTED: &str = r#"<?xml version='1.0' encoding='utf-8' ?>
<PageContents xmlns='http://schemas.microsoft.com/office/visio/2012/main' xml:space='preserve'><Shapes>
<Shape ID='1' NameU='Router' Name='Router' Type='Shape' Master='5'><Cell N='PinX' V='1'/><Cell N='PinY' V='8'/><Cell N='Width' V='0.6'/><Cell N='Height' V='0.6'/>
<Section N='Character'><Row IX='0'><Cell N='Color' V='#ff0000' F='THEMEGUARD(RGB(255,0,0))'/><Cell N='Size' V='0.1666666666666667'/><Cell N='Style' V='3'/></Row></Section>
<Section N='Para'><Row IX='0'><Cell N='HorzAlign' V='0'/></Row></Section><Text>EDGE-RTR1</Text></Shape>
<Shape ID='2' NameU='Switch' Name='Switch' Type='Shape' Master='6'><Cell N='PinX' V='4'/><Cell N='PinY' V='6'/><Cell N='Width' V='0.6'/><Cell N='Height' V='0.6'/><Text>CORE-SW1</Text></Shape>
<Shape ID='3' NameU='Dynamic connector' Name='Dynamic connector' Type='Shape' Master='2'><Cell N='PinX' V='2.65' F='Inh'/><Cell N='PinY' V='7' F='Inh'/><Cell N='Width' V='3.3' F='GUARD(EndX-BeginX)'/><Cell N='Height' V='-2' F='GUARD(EndY-BeginY)'/><Cell N='LocPinX' V='1.65' F='Inh'/><Cell N='LocPinY' V='-1' F='Inh'/><Cell N='BeginX' V='1' F='_WALKGLUE(BegTrigger,EndTrigger,WalkPreference)'/><Cell N='BeginY' V='8'/><Cell N='EndX' V='4.3'/><Cell N='EndY' V='6'/>
<Section N='Geometry' IX='0'><Row T='MoveTo' IX='1'><Cell N='Y' V='0'/></Row><Row T='LineTo' IX='2'><Cell N='X' V='0'/><Cell N='Y' V='-1'/></Row><Row T='LineTo' IX='3'><Cell N='X' V='3.3'/></Row><Row T='LineTo' IX='4'><Cell N='Y' V='-2'/></Row><Row T='LineTo' IX='5' Del='1'/></Section></Shape>
</Shapes><Connects><Connect FromSheet='3' FromCell='EndX' FromPart='12' ToSheet='2' ToCell='PinX' ToPart='3'/><Connect FromSheet='3' FromCell='BeginX' FromPart='9' ToSheet='1' ToCell='PinX' ToPart='3'/></Connects></PageContents>"#;

    #[test]
    fn a_routed_connector_keeps_its_bends_and_a_label_its_formatting() {
        let (page, _) = parse_page(ROUTED, "Page-1").unwrap();
        assert_eq!(page.links.len(), 1);
        let link = &page.links[0];
        let near = |a: [f64; 2], b: [f64; 2]| (a[0] - b[0]).abs() < 1e-9 && (a[1] - b[1]).abs() < 1e-9;
        assert_eq!(link.waypoints.len(), 2, "{:?}", link.waypoints);
        assert!(near(link.waypoints[0], [1.0, 7.0]), "down from the router: {:?}", link.waypoints);
        assert!(near(link.waypoints[1], [4.3, 7.0]), "across, the omitted Y carried: {:?}", link.waypoints);

        let router = page.devices.iter().find(|d| d.label == "EDGE-RTR1").unwrap();
        assert_eq!(
            router.label_style,
            Some(TextStyle { bold: true, italic: true, size: Some(12.0), color: "#ff0000".into(), align: "left".into() })
        );
        assert_eq!(page.devices.iter().find(|d| d.label == "CORE-SW1").unwrap().label_style, None, "nothing stated, nothing carried");
    }

    /// Every connector in a real drawing starts and finishes its computed path
    /// on its own `BeginX`/`EndX`: `CV_VSDX=drawing.vsdx cargo test -p
    /// coreview real_connectors -- --ignored --nocapture`.
    #[test]
    #[ignore]
    fn real_connectors_start_and_end_where_visio_says() {
        let bytes = std::fs::read(std::env::var("CV_VSDX").expect("CV_VSDX")).unwrap();
        let mut zip = zip::ZipArchive::new(std::io::Cursor::new(bytes)).unwrap();
        let names: Vec<String> = zip.file_names().filter(|n| n.starts_with("visio/pages/page") && n.ends_with(".xml")).map(str::to_string).collect();
        let mut checked = 0;
        for name in names {
            let mut buf = Vec::new();
            ...
            for shape in doc.descendants().filter(|n| n.has_tag_name("Shape")) {
                let num = |c: &str| cell(shape, c).and_then(|v| v.parse::<f64>().ok());
                let (Some(bx), Some(by), Some(ex), Some(ey)) = (num("BeginX"), num("BeginY"), num("EndX"), num("EndY")) else { continue };
                let path = shape_path(shape);
                let (Some(first), Some(last)) = (path.first(), path.last()) else { continue };
                println!("{name} shape {}: {} points, begin off by {:.2e}, end off by {:.2e}", shape.attribute("ID").unwrap_or(""), path.len(), (first.0 - bx).hypot(first.1 - by), (last.0 - ex).hypot(last.1 - ey));
                assert!((first.0 - bx).hypot(first.1 - by) < 1e-6 && (last.0 - ex).hypot(last.1 - ey) < 1e-6);
                checked += 1;
            }
        }
        assert!(checked > 0, "no connectors with geometry in the drawing");
    }

    #[test]
    fn a_straight_connector_has_no_bends() {
        let straight = ROUTED.replace("<Row T='LineTo' IX='2'><Cell N='X' V='0'/><Cell N='Y' V='-1'/></Row><Row T='LineTo' IX='3'><Cell N='X' V='3.3'/></Row><Row T='LineTo' IX='4'><Cell N='Y' V='-2'/></Row>", "<Row T='LineTo' IX='2'><Cell N='X' V='3.3'/><Cell N='Y' V='-2'/></Row>");
        let (page, _) = parse_page(&straight, "Page-1").unwrap();
        assert!(page.links[0].waypoints.is_empty());
    }
