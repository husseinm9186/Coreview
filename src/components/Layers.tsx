...
export function Layers() {
  ...
  const addStandardLayers = useStore((s) => s.addStandardLayers);
  const [adding, setAdding] = useState('');

  const layers = layersOf(canvas.layers);
  const hidden = layers.filter((l) => !l.visible).length;
  const missing = STANDARD_LAYER_NAMES.filter((name) => !layers.some((l) => l.name.toLowerCase() === name.toLowerCase()));
  ...
  return (
    <details className="cv-layers" open={open || undefined}>
      ...
      <div className="cv-layers-list">
        {layers.map((layer) => (
          <div key={layer.id} className="cv-layer">
            ...
            <button
              type="button"
              className={`cv-layer-print${layer.print === false ? ' is-off' : ''}`}
              title={layer.print === false ? 'Include this view in exports and printing' : 'Leave this view out of exports and printing'}
              aria-label={layer.print === false ? `Print ${layer.name}` : `Do not print ${layer.name}`}
      ...
      {missing.length > 0 && (
        <button type="button" className="cv-btn cv-btn-small cv-layers-standard" onClick={() => addStandardLayers()}>
          Add {missing.join(', ')}
        </button>
      )}
      <p className="cv-help">
