...
export function Palette() {
  ...
  return (
    <aside className="cv-palette" aria-label="Device palette">
      ...
      {/* LT-166: sections that say what kind of area they are. */}
      {(() => {
        const kinds = BOUNDARY_KINDS.filter(
          (k) => !q || BOUNDARIES[k].label.toLowerCase().includes(q) || k.includes(q),
        );
            ...
            <div className="cv-palette-grid">
              {kinds.map((k) => (
                <button
                  ...
                  className="cv-palette-item"
                  draggable
                  onDragStart={(e) => drag(e, `boundary:${k}`)}
                  title={`Drag a ${BOUNDARIES[k].label} on to the canvas`}
                >
                  <Icon
                    className="cv-palette-icon"
...
/** Stencils bundled as a resource. None ship since D-028 — Coreview's own
 *  shapes are the palette groups above — so this section shows only if a
 *  build carries some. Absent in the browser build, where there is no
 *  resource to read. */
...
/** Removing a bundled stencil pack (LT-103) — none ship since D-028 — frees the space
