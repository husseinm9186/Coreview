...
import { ipc, isDesktop, type ImportedDevice, type ImportedPage, type VisioImport } from '../lib/ipc';
...
interface DraftDevice {
  ...
  width: number;
  height: number;
  labelStyle?: ImportedDevice['labelStyle'];
}
...
export function VisioImportPanel() {
  ...
  const pick = async () => {
    ...
    try {
      ...
      // LT-244: a draw.io drawing reads into the same preview.
      const out: VisioImport = /\.(drawio|xml)$/i.test(path) ? await ipc.importDrawio(path) : await ipc.importVisio(path);
  ...
  const add = () => {
    ...
    draft.forEach((page, pageIndex) => {
      ...
      for (const d of page.devices) {
        ...
        // LT-243: the name set the way the drawing set it — points to pixels.
        const ls = d.labelStyle;
        if (ls) {
          data.labelStyle = {
            ...(ls.bold ? { bold: true } : {}),
            ...(ls.italic ? { italic: true } : {}),
            ...(ls.size ? { size: Math.round((ls.size * 4) / 3) } : {}),
            ...(ls.color ? { color: ls.color } : {}),
            ...(ls.align === 'left' || ls.align === 'center' || ls.align === 'right' ? { align: ls.align } : {}),
          };
        }
    ...
    <div className="cv-import">
      <div className="cv-row cv-row-tight">
        <button type="button" className="cv-btn" disabled={!isDesktop || busy} onClick={pick}>
          {busy ? 'Reading…' : isDesktop ? 'Choose a drawing' : 'Desktop app only'}
        </button>
        {draft && (
          <>
