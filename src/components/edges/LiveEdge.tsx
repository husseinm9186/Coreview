...
import { cableTag } from '../../lib/cables';
import { cssOf } from '../../lib/textStyle';
import { obstaclesFor, routeAround, routePath, type Side as RouteSide } from '../../lib/avoidRoute';
...
function portAnchors(edgePath: string, rank: number): { s: DOMPoint; t: DOMPoint } | null {
  try {
    const p = document.createElementNS('http://www.w3.org/2000/svg', 'path');
    p.setAttribute('d', edgePath);
    const len = p.getTotalLength();
...
function LiveEdgeInner(props: EdgeProps) {
  ...
  // LT-189: narrow reads only. This used to subscribe to the whole document,
  ...
  // An end being dragged is read from the drag overlay (LT-189), so the link
  ...
  const [anchorDrag, setAnchorDrag] = useState<{
    ...
    x: number;
    y: number;
    /** LT-176: the device and connection point the end has snapped to. */
    snapTo?: string;
  ...
  // LT-178: a link routed round devices needs to know where they are. Only such
  // a link subscribes to the page's devices; every other link reads nothing.
  const avoiding = data.pathType === 'avoid';
  ...
  }, [data.pathType, data.lane, data.curvature, curveDrag, wps, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, avoiding, obstacleNodes]);
  ...
  // LT-189: the fade itself is a stylesheet rule (TraceFade), so pointing at a
  ...
  // its parallel neighbour appears. LT-189: only for a link with port chips to
  ...
  const anchors = useMemo(
    () => (wantsPortChips ? portAnchors(edgePath, rank) : null),
    [wantsPortChips, edgePath, rank],
  );
  ...
  const beginLabelDrag = useCallback(
    // LT-189: the start may be a function, worked out on press. Worked out on
  ...
  const dragAnchorEnd = (which: 'source' | 'target') => (e: React.PointerEvent) => {
    ...
    e.stopPropagation();
    e.preventDefault();
    // LT-176: near a connection point of the device under the pointer — or,
    ...
    const connectionUnder = (f: { x: number; y: number }) => {
      ...
      for (const n of own ? [...candidates, own] : candidates) {
        const box = boxOf(n);
    ...
    const move = (ev: PointerEvent) => {
      ...
      setAnchorDrag(
        snap ? { which, x: snap.at.x, y: snap.at.y, snapTo: snap.nodeId, anchor: snap.anchor } : { which, x: f.x, y: f.y },
      );
    };
    const up = () => {
      window.removeEventListener('pointermove', move);
      ...
      // LT-157: released over a different device, that end moves there. Over
      // the link's own other end it stays put rather than looping back.
      // Anywhere else it slides round its own device, as LT-098 always did.
      ...
      if (over) {
        if (over.id !== otherId) {
          useStore.getState().reconnectEdge(id, which, over.id, final.snapTo === over.id ? final.anchor : undefined);
        }
        return;
      }
      ...
      <EdgeLabelRenderer>
        ...
        {data.label || cableTag(data) ? (
          ...
          >
            ...
            {/* LT-167: what the cable is, ahead of what it is for. */}
            {cableTag(data) && (
