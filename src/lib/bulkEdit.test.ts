...
describe('shared text fields (LT-236)', () => {
  it('agree, disagree, or count a blank as a value of its own', () => {
    const s = describeSelection([device('a', { role: 'Access', site: 'HQ' }), device('b', { role: 'Access' })]);
    expect(s.role).toEqual({ kind: 'same', value: 'Access' });
    expect(s.site).toEqual({ kind: 'mixed' });
    expect(s.rack).toEqual({ kind: 'same', value: '' });
  });
});
