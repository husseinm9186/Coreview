...
export interface Selection {
  ...
  /** LT-236: the text fields a crawl or an estate shares across devices. An
   *  empty field counts as a value, so "some have a site, some do not" is
   *  mixed rather than the site of the ones that have it. */
  role: Shared<string>;
  site: Shared<string>;
  rack: Shared<string>;
  vendor: Shared<string>;
...
export function describeSelection(nodes: TopoNode[]): Selection {
  ...
  return {
    ...
    role: shared(data.map((d) => d.role ?? '')),
    site: shared(data.map((d) => d.site ?? '')),
    rack: shared(data.map((d) => d.rack ?? '')),
    vendor: shared(data.map((d) => d.vendor ?? '')),
