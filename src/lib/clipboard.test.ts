...
describe('pasteClipping', () => {
  ...
  it('takes the copy out of the rack U the original is in, keeping its rack (LT-195)', () => {
    const clip = copySelection([node('a', true, { rack: 'R1', rackU: 12, rackUnits: 2 })], []);
    const data = pasteClipping(clip, { x: 0, y: 0 }, ids).nodes[0]!.data as { rack?: string; rackU?: number; rackUnits?: number };
    expect(data.rackU).toBeUndefined();
    expect(data.rack).toBe('R1');
