...
export function pasteClipping(
  ...
  const nodes = clip.nodes.map((n) => {
    ...
    // A copy is another box, and another box cannot be in the same U of the
    // same rack (LT-195): it keeps its rack and height and waits to be placed.
    if ('rackU' in data) delete data.rackU;
    return {
      ...n,
      id: remap.get(n.id)!,
