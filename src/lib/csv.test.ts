...
describe('writing the diagram back out', () => {
  ...
  it('carries every fact discovery can learn, for profiling (LT-147)', () => {
    // The operator asked for this to include "all the info we disover like
    // mac, name , ip, vendor ... ect" because he reads it to profile an
    // estate, not only to re-import a diagram.
    const csv = nodesToCsv([
      {
        label: 'LAB-CORE-SW1', type: 'switch', address: '192.168.77.7',
        probeType: 'icmp', tags: ['core'],
        vendor: 'Cisco Systems', model: 'WS-C2960CX-8PC-L',
        serial: 'FOC0000TEST', assetTag: 'ASSET-1',
        hostname: 'LAB-CORE-SW1.lab.local', mac: 'cc:7f:75:00:00:03', vlan: '10',
        osVersion: 'Cisco IOS Software, C2960CX Software, Version 15.2(7)E',
        switchPort: 'CORE-SW Gi1/0/1', openPorts: '22/SSH, 443/HTTPS',
        discoveredVia: 'Logged in', site: 'HQ', rack: 'R1', role: 'access',
      },
    ]);
    const [header, row] = csv.split('\r\n');
    for (const column of ['MAC address', 'VLAN', 'Software', 'Connects to',
                          'Open ports', 'Found by', 'Hostname', 'Serial number']) {
      expect(header, `missing column ${column}`).toContain(column);
    }
    expect(row).toContain('cc:7f:75:00:00:03');
    expect(row).toContain('FOC0000TEST');
    expect(row).toContain('22/SSH');
    expect(row).toContain('Logged in');
    // A value with a comma in it must not shift every column after it.
    expect(row).toContain('"Cisco IOS Software, C2960CX Software, Version 15.2(7)E"');
  });

  it('still gives a device nothing is known about its own row', () => {
    // A row per device, even an empty one: a profile with holes is useful and
    // a profile that silently drops hosts is not.
    const csv = nodesToCsv([
      { label: 'Unknown box', type: 'generic', address: '', probeType: 'manual', tags: [] },
    ]);
    const rows = csv.split('\r\n');
    expect(rows).toHaveLength(2);
    expect(rows[1]).toContain('Unknown box');
  });
