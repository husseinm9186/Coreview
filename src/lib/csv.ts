...
export interface NodeCsvRow {
  ...
  serial?: string;
  assetTag?: string;
  /** LT-245, LT-247: where it is and what it does, from a mapped spreadsheet
   *  or a NetBox export. */
  role?: string;
  site?: string;
  rack?: string;
...
export function nodesToCsv(
  nodes: {
    ...
    hostname?: string; mac?: string; vlan?: string; osVersion?: string;
    switchPort?: string; openPorts?: string; discoveredVia?: string;
    site?: string; rack?: string; role?: string;
    stackKind?: string; stackMembers?: string;
  }[],
): string {
  // LT-147: every column discovery can know, because the operator wants this
  // for profiling and not only for re-importing a diagram. New columns go on
  // the end, so a spreadsheet built against the old ones still lines up and
  // the header-driven importer reads either.
  return toCsv([
    ['Name', 'Type', 'IP', 'Probe type', 'Port', 'Notes', 'Tags',
     'Vendor', 'Model', 'Serial number', 'Asset tag',
     'Hostname', 'MAC address', 'VLAN', 'Software', 'Connects to',
     'Open ports', 'Found by', 'Site', 'Rack', 'Role',
     'Stack', 'Stack members'],
    ...nodes.map((n) => [
      n.label,
      n.type,
      ...
      n.role ?? '',
      n.stackKind ?? '',
      // One per line in the app; semicolons in a spreadsheet cell, which is
      // the only separator this format handles without quoting games.
      (n.stackMembers ?? '').split('\n').filter(Boolean).join('; '),
    ]),
  ]);
}
