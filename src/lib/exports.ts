import { cableScheduleMarkdown, type CableRow } from './cableSchedule';
...
export interface ReportInput {
  ...
  /** LT-198: the cable schedule, when the project has links to list. */
  cables?: CableRow[];
  /** LT-253: each page's drawing, embedded in the file as an image, so the
   *  report is one file that still shows the network when it is sent on. */
...
${r.diagrams.map((d) => `### ${d.name.replace(/[\r\n]+/g, ' ')}\n\n![${d.name.replace(/[[\]\r\n]/g, ' ')}](data:image/svg+xml;base64,${utf8ToBase64(d.svg)})`).join('\n\n')}
`
    : ''
}
${
  r.cables
    ? `
## Cable schedule (${r.cables.length})

${cableScheduleMarkdown(r.cables)}
`
    : ''
}
---
