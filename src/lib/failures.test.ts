...
describe('reasonWithoutAddress', () => {
  it('drops an address the reason repeats at the front', () => {
    expect(reasonWithoutAddress('192.168.77.112', '192.168.77.112 rejected the credentials')).toBe(
  ...
  it('leaves a reason that names the host mid-sentence alone', () => {
    ...
    const reason = 'could not reach 192.168.77.9:22: connection refused';
    expect(reasonWithoutAddress('192.168.77.9', reason)).toBe(reason);
  });
  ...
  it('does not strip a host that is only a prefix of a longer one', () => {
    ...
    const reason = '192.168.77.11 rejected the credentials';
    expect(reasonWithoutAddress('192.168.77.1', reason)).toBe(reason);
  });
