...
describe('connection points (LT-176, LT-179)', () => {
  const box = { x: 100, y: 100, w: 80, h: 40 };

  it('offers three along each side and the centre', () => {
    expect(CONNECTION_POINTS).toHaveLength(13);
    expect(CONNECTION_POINTS).toContainEqual({ x: 0.5, y: 0.5 });
    for (const a of CONNECTION_POINTS.filter((p) => !(p.x === 0.5 && p.y === 0.5))) {
      expect(a.x === 0 || a.x === 1 || a.y === 0 || a.y === 1, JSON.stringify(a)).toBe(true);
    }
  });

  it('snaps to the nearest point within the tolerance', () => {
    expect(snapToConnection(box, { x: 141, y: 103 }, 6)).toEqual({ x: 0.5, y: 0 });
    expect(snapToConnection(box, { x: 139, y: 121 }, 6)).toEqual({ x: 0.5, y: 0.5 });
    expect(snapToConnection(box, { x: 181, y: 111 }, 6)).toEqual({ x: 1, y: 0.25 });
  });

  it('does not snap from beyond the tolerance', () => {
    expect(snapToConnection(box, { x: 128, y: 100 }, 6)).toBeNull();
  });
});
