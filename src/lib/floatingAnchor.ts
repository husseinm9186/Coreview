...
/**
 * The connection points every shape offers (LT-179): a quarter, halfway and
 * three quarters along each side, and the centre. A link end dragged near one
 * snaps to it (LT-176), so two links can meet a device at exactly the same
 * place and a hub can be wired from its middle.
 */
export const CONNECTION_POINTS: readonly Anchor[] = [
  { x: 0.25, y: 0 }, { x: 0.5, y: 0 }, { x: 0.75, y: 0 },
  { x: 1, y: 0.25 }, { x: 1, y: 0.5 }, { x: 1, y: 0.75 },
  { x: 0.25, y: 1 }, { x: 0.5, y: 1 }, { x: 0.75, y: 1 },
  { x: 0, y: 0.25 }, { x: 0, y: 0.5 }, { x: 0, y: 0.75 },
  { x: 0.5, y: 0.5 },
];

/** The connection point of `box` nearest `point`, if it is within `tolerance`
 *  (diagram units — divide screen pixels by the zoom first); otherwise none. */
export function snapToConnection(box: Box, point: { x: number; y: number }, tolerance: number): Anchor | null {
  let best: Anchor | null = null;
  let bestD = Infinity;
  for (const a of CONNECTION_POINTS) {
