 ...
 * 4. **What the device itself said** (LT-145). Where a crawl read a default
...
export interface HierarchyEdge {
  source: string;
  target: string;
  /** Which way traffic proven to leave by this link goes (LT-145).
   *
   *  Set only where a crawl read the device's own default route (LT-131), so
...
export function tiersFor(nodes: HierarchyNode[], edges: HierarchyEdge[]): Map<string, number> {
  ...
  // Rule 4 (LT-145): a proven direction outranks all three above.
  //
  // The rules so far are the glyph, the cabling and a fallback. None of them
  // is the device's own forwarding decision, and since LT-131 a link carries
  ...
  // this wins — which is the whole of LT-145.
  //
