...
describe('straightRuns', () => {
  ...
  it('cuts a curve into pieces so it can hop too (LT-161)', () => {
    // Until LT-161 a curve had no runs at all, so bezier links never hopped.
    const runs = straightRuns('M0,0C20,0 80,60 100,60');
    expect(runs.length).toBeGreaterThan(10);
    expect(runs[0]).toMatchObject({ x: 0, y: 0, curved: true });
    expect(runs[runs.length - 1]).toMatchObject({ x2: 100, y2: 60 });
  });
