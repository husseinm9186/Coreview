...
describe('linkDefaults (LT-079)', () => {
  ...
  it('resetting restores the look and clears a hand-drawn route', () => {
    ...
    expect(patch.pathType).toBe(BUILT_IN_LINK_STYLE.pathType);
    expect(patch.pathType).toBe('bezier');
    expect(patch.waypoints).toEqual([]);
    expect(patch.curvature).toBeUndefined();
  });

  it('carries a link\'s own arrowheads into the default style, and back on reset (LT-180)', () => {
    const own = { ...link(), startCap: 'diamond', endCap: 'open-arrow' } as never;
    expect(styleOf(own)).toMatchObject({ startCap: 'diamond', endCap: 'open-arrow' });
    const patch = resetToDefault({ startCap: 'circle', endCap: 'square' });
    expect(patch.startCap).toBe('circle');
    expect(patch.endCap).toBe('square');
    const plain = resetToDefault(undefined);
    expect(plain.startCap).toBeUndefined();
    expect(plain.endCap).toBeUndefined();
  });
