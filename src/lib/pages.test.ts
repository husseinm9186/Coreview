...
describe('duplicatePage', () => {
  ...
  it('takes the copy out of the original’s rack U (LT-195)', () => {
    const n1 = node('n1');
    const source = doc([page('a', 'A', { nodes: [{ ...n1, data: { ...n1.data, rack: 'R1', rackU: 3 } } as TopoNode] })]);
    const d = duplicatePage(source, 'a', (() => {
      let n = 0;
      return () => `fresh-${n++}`;
    })());
    expect((d.pages[1]!.nodes[0]!.data as { rackU?: number }).rackU).toBeUndefined();
    expect((d.pages[0]!.nodes[0]!.data as { rackU?: number }).rackU).toBe(3);
  });
...
describe('the cached page indexes (LT-189)', () => {
  const node = (id: string) => ({ id, type: 'device', position: { x: 0, y: 0 }, data: {} }) as never;
  const edge = (id: string) => ({ id, source: 'a', target: 'b', data: {} }) as never;
  const docOf = (pages: { id: string; nodes: never[]; edges: never[] }[]) =>
    ({ pages: pages.map((p) => ({ ...p, name: p.id, canvas: {} })), activePageId: pages[0]!.id, probes: [] }) as never;

  it('finds a node or link on any page by id', () => {
    const doc = docOf([
      { id: 'p1', nodes: [node('a')], edges: [edge('x')] },
      { id: 'p2', nodes: [node('b')], edges: [edge('y')] },
    ]);
    expect(nodeById(doc, 'b')?.id).toBe('b');
    expect(edgeById(doc, 'y')?.id).toBe('y');
    expect(nodeById(doc, 'nope')).toBeUndefined();
  });

  it('answers the same array for the same pages, and a new one when a page changes', () => {
    const doc = docOf([{ id: 'p1', nodes: [node('a')], edges: [] }]) as { pages: { nodes: unknown[] }[] };
    const first = allNodes(doc as never);
    expect(allNodes(doc as never)).toBe(first);
    const changed = { ...doc, pages: [{ ...doc.pages[0]!, nodes: [node('a'), node('c')] }] };
    expect(allNodes(changed as never)).not.toBe(first);
    expect(allNodes(changed as never).map((n) => n.id)).toEqual(['a', 'c']);
    expect(nodeById(changed as never, 'c')?.id).toBe('c');
    // The old document still answers for itself.
    expect(nodeById(doc as never, 'c')).toBeUndefined();
  });
});
