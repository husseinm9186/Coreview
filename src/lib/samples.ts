...
import { uid } from './id';
import { PROBE_DEFAULTS, type DeviceInventory, type DeviceNodeData, type DeviceType, type InventoryPort, type LinkHealthRuleType, type Probe } from '../types/domain';
...
/** The branch office drawing, shared by its sample and the guided tour. */
function branchOffice(): ProjectDocument {
  return (
      build(
        [
...
/** LT-271: the branch office with invented crawl results on its switches, so
 *  every step of the tour has something to show, and the tour itself. */
function guidedTour(): ProjectDocument {
  const doc = branchOffice();
  const port = (name: string, status: string, vlan: number | undefined, extra: Partial<InventoryPort> = {}): InventoryPort => ({
    port: name, status, speed: status === 'connected' ? '1000' : 'auto', duplex: status === 'connected' ? 'full' : 'auto',
    mode: vlan === undefined ? 'trunk' : 'access', vlan, errors: { input: 0, crc: 0, output: 0, collisions: 0, resets: 0, drops: 0 }, ...extra,
  });
  const inventory = (ports: InventoryPort[]): DeviceInventory => ({
    collectedAt: 0, ports, routes: [], spanningTree: [],
    vlans: [{ id: 10, name: 'STAFF' }, { id: 20, name: 'WIRELESS' }, { id: 30, name: 'SERVERS' }],
  });
  for (const node of doc.pages[0]!.nodes) {
    const d = node.data as DeviceNodeData;
    if (d.label === 'Core switch') {
      d.inventory = inventory([
        port('Gi1/0/1', 'connected', undefined, { trunkVlans: '10,20' }),
        port('Gi1/0/2', 'connected', 20),
        port('Te1/0/48', 'connected', undefined, { trunkVlans: '10,20,30', speed: '10G' }),
      ]);
      d.serial = 'FOC0000X001';
      d.model = 'C9300-48P';
    }
    if (d.label === 'Access switch 1') {
      d.inventory = inventory([
        // Up, but dropping frames: the thing a link that looks green hides.
        port('Gi1/0/48', 'connected', undefined, { trunkVlans: '10,20', errors: { input: 812, crc: 790, output: 0, collisions: 0, resets: 3, drops: 41 } }),
        port('Gi1/0/10', 'connected', 30),
        port('Gi1/0/11', 'notconnect', 10),
      ]);
      d.serial = 'FOC0000X002';
      d.model = 'C9200L-48P-4G';
    }
  }
  doc.guide = { done: [] };
  return doc;
...
export const SAMPLES: Array<{
  ...
  {
    name: 'Sample — Guided tour',
    description: 'A branch office with a checklist beside it: pick a device, read a link, run validation, search and make a report.',
    build: () => guidedTour(),
  },
  {
