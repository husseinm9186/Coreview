...
describe('subnetOf', () => {
  it('reduces an address to its /24', () => {
    expect(subnetOf('192.168.77.7')).toBe('192.168.77.0/24');
...
describe('groupBySubnet', () => {
  it('puts each network in its own group', () => {
    const g = groupBySubnet([
      device('a', '192.168.77.7'),
      device('b', '192.168.77.9'),
      device('c', '10.2.80.1'),
      device('d', '10.2.80.2'),
    ]);
    expect(g.subnets).toEqual(['10.2.80.0/24', '192.168.77.0/24']);
  ...
  it('leaves a subnet holding one device alone', () => {
    ...
    const g = groupBySubnet([device('a', '192.168.77.7'), device('b', '10.9.9.9')]);
