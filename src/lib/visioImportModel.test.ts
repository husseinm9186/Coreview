...
describe('importedLinkData', () => {
  it('draws an imported link as a curve, whatever the diagram default is', () => {
    ...
    //
    // The built-in default became bezier too (LT-130), so asserting against
    // it no longer shows this is independent of it. Handing in a default that
    // is deliberately something else does.
    expect(link().pathType).toBe('bezier');
    const orthogonal = importedLinkData(
      { ...BUILT_IN_LINK_STYLE, pathType: 'step' },
      { sourcePort: '', targetPort: '', color: '' },
    );
    expect(orthogonal.pathType).toBe('bezier');
  });
...
describe('a routed connector (LT-243)', () => {
  it('keeps the drawing\'s bends as waypoints on a stepped path', () => {
    const style = BUILT_IN_LINK_STYLE;
    const routed = importedLinkData(style, { sourcePort: 'Gi0/1', targetPort: 'Gi0/2', color: '', waypoints: [{ x: 10, y: 20 }, { x: 40, y: 20 }] });
    expect(routed.waypoints).toEqual([{ x: 10, y: 20 }, { x: 40, y: 20 }]);
    expect(routed.pathType).toBe('step');
    const straight = importedLinkData(style, { sourcePort: '', targetPort: '', color: '', waypoints: [] });
    expect(straight.waypoints).toBeUndefined();
    expect(straight.pathType).toBe('bezier');
  });
});
