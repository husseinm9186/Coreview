...
export interface DraftLinkShape {
  ...
  targetPort: string;
  color: string;
  /** LT-243: the drawing's bends, already on the canvas's scale. */
...
export function importedLinkData(style: LinkStyleDefaults, l: DraftLinkShape): LinkData {
  ...
  return {
    ...
    // A route the drawing states is kept, bend for bend, rather than replaced
    // by a curve.
    ...(l.waypoints?.length ? { waypoints: l.waypoints, pathType: 'step' as const } : {}),
    enabled: true,
    maintenance: false,
