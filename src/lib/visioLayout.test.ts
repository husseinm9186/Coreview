...
import { BASE_SCALE, placeDevices, placeDrawing, scaleFor } from './visioLayout';
...
describe('placeDrawing (LT-243)', () => {
  it('moves a bend by the same transform as the devices around it', () => {
    const a = device('a', { x: 1, y: 8, width: 1, height: 1 });
    const b = device('b', { x: 4, y: 6, width: 1, height: 1 });
    const { placed, point } = placeDrawing([a, b]);
    const centre = (id: string) => {
      const p = placed.get(id)!;
      return { x: p.x + p.width / 2, y: p.y + p.height / 2 };
    };
    // A bend straight below a and level with b sits at a's x and b's y.
    const bend = point(1, 6);
    expect(Math.abs(bend.x - centre('a').x)).toBeLessThanOrEqual(1);
    expect(Math.abs(bend.y - centre('b').y)).toBeLessThanOrEqual(1);
  });
});
