...
export function placeDevices(devices: ImportedDevice[]): Map<string, PlacedDevice> {
  return placeDrawing(devices).placed;
}

/**
 * The devices placed, and the same move from drawing inches to canvas pixels
 * for anything else on the page — a connector's bends (LT-243) — so a route
 * drawn between two devices still runs between them.
 */
export function placeDrawing(devices: ImportedDevice[]): { placed: Map<string, PlacedDevice>; point: (x: number, y: number) => { x: number; y: number } } {
  const out = new Map<string, PlacedDevice>();
  if (devices.length === 0) return { placed: out, point: (x, y) => ({ x: x * BASE_SCALE, y: -y * BASE_SCALE }) };
  ...
  return { placed: out, point: (x, y) => ({ x: Math.round((x - left) * scale - minX), y: Math.round((top - y) * scale - minY) }) };
}
